...
var VideoF = function()
{
	...
	this.create = function()
	{
		...
		$(video).css({
			'width' : config['width'],
			'height' : config['mode'] == 'showall' ? config['height'] : '',
			'position' : 'absolute',
			'top' : '0',
			'left' : '0',
			'display' : 'block'
		...
		//append
		$('#'+config['container']).append(video);
	},

	this.addEvt = function()
	{
		$(config['bind']).on('touchend', function(){
			self.ready();
		});
	},
	...
	{
		this.play();
		if (typeof config['begin'] != 'undefined') 
		{
			config['begin']();
		}
		video.addEventListener('timeupdate', function(){
			...
			{
                self.over();
                if (typeof config['playing'] != 'undefined') 
                {
                	config['playing']();
                }
	...
	this.over = function()
	{
		video.addEventListener('timeupdate', this.end);
	},
	...
	this.skip = function()
	{
		$('#'+config['container']).hide();
        self.pause();
        video.removeEventListener('timeupdate', this.end, false);
        if (typeof config['over'] != 'undefined')
        {
        	 config['over']();
        	 video.currentTime = 0;
        } 
